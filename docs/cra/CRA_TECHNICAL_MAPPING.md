...
| Detect | Vulnerability monitoring, security-relevant event handling | Turns a disclosed CVE into a runtime detection rule (auditd + OCSF), shipped independently of firmware so detection coverage doesn't wait for a full release cycle. | [`detection-event.md`](../evidence/detection-event.md), [`docs/detect.md`](../detect.md). |
| Update & Boot | Protection from unauthorized modification, secure update delivery | U-Boot FIT signature verification and signed A/B firmware updates (swupdate), plus independently signed detection-rule updates. | [`fit-verification.md`](../evidence/fit-verification.md), [`signed-ab-update.md`](../evidence/signed-ab-update.md), [`signed-rule-update.md`](../evidence/signed-rule-update.md). |
